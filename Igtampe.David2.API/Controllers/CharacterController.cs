...
namespace Igtampe.Toffee.Backend.Controllers {
    ...
    public class CharacterController : ControllerBase {
        ...
        private readonly CharacterAgent agent;

        public CharacterController(DavidContext Context)
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromHeader] Guid? SessionID, [FromBody] CharacterRequest request)
            => Ok(await agent.Create(SessionID, request));

        [HttpPut("{id}")]
        public async Task<IActionResult> Update([FromHeader] Guid? SessionID, [FromRoute] Guid id, [FromBody] CharacterRequest request)
